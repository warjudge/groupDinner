...
<template>
  <scroll class="wrap" ref="scroll"  @pullingUp="handleGetHistoryList">
    <iou-history-template billUrl="/restaurant-iou-bill">
...
import { getIouRecordList } from '@/service/iou-restaurant.js';
...
export default {
  ...
  methods: {
    async handleGetHistoryList() {
      ...
      const res = await getIouRecordList({ id: 3, pageNum, pageSize });
      if (res.code == 200) {
