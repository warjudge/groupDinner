<template>
  <div>
    <header>
      <x-icon type="ios-arrow-left" class="x_icon" size="0.8rem" @click="go2router('/license-upload-case')"></x-icon>
      <span></span>
    </header>
    <div class="title">许可证照上传</div>
...
<script>
  import {submitlicense, submitlicenseUpdate} from '../../service/apply-company.js'
export default {
  data () {
    return {
      titleText:'',
  ...
  methods:{
    ...
    async submit(){
      if(this.isReapply){
        const res1 = await submitlicenseUpdate({
          permission:this.file
        });
        console.log('res', res1);
        if(res1.code=='200'){
          sessionStorage.license=true;
          this.$router.push({path:'/company-qualification-info',query:{reapply:true}});
        }
      }else if(this.isRefill){
        const res2 = await submitlicense({
          permission:this.file
        });
        console.log('res', res2);
        if(res2.code=='200'){
          this.$router.push({path:'/company-qualification-info',query:{refill:true}});
        }
      }else{
        const res3 = await submitlicense({
          permission:this.file
        });
        console.log('res', res3);
        if(res3.code=='200'){
          this.$router.push({path:'/company-qualification-info'});
        }
      }
    },
    getFile ($event){
