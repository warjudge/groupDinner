...
<script>
  import {viewAptitudeFill} from '../../service/apply-company.js'
export default {
  components: {
  ...
  data () {
    return {
      ...
      status:{
        bussiness:false,
        busFill:'上传营业执照',
        busPath:{path:'/company-businessLicense-upload-case'},
        idcard:false,
        idFill:'上传身份证',
        idPath:{path:'/company-idcard-upload'},
        license:false,
        licFill:'上传许可证',
        licPath:{path:'/company-license-upload-case'},
      },
      btnText:'下一步',
    }
  },
  mounted(){
    if(this.$route.query.reapply){
      ...
      this.status.busPath={path:'/company-businessLicense-upload-case',query:{reapply:true}};
      this.status.idPath={path:'/company-idcard-upload',query:{reapply:true}};
      this.status.licPath={path:'/company-license-upload-case',query:{reapply:true}};
      ...
      this.status.busPath={path:'/company-businessLicense-upload-case',query:{refill:true}};
      this.status.idPath={path:'/company-idcard-upload',query:{refill:true}};
      this.status.licPath={path:'/company-license-upload-case',query:{refill:true}};
      this.btnText='确定';
  ...
  methods:{
    ...
    submit(){
      if(this.isReapply){
        ...
        this.$router.push({path:'/company-apply-result-error',query:{isReapply:true}});
      }else if(this.isRefill){
        this.$router.push({path:'/company-apply-result',query:{refill:true}});
      }else{
        this.$router.push({path:'/company-receipt-info'});
      }
    },
