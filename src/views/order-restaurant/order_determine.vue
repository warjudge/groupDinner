<template>
  <div>
    <header>
      <x-icon type="ios-arrow-left" class="x_icon" size="0.8rem" @click="go2router('/receiptInfo')"></x-icon>
      <span>
      </span>
    </header>
    ...
    <ul>
      <li>
        <order-adress-detail></order-adress-detail>
      </li>
      <li>
        <order-item-detail :details="data"></order-item-detail>
      </li>
    </ul>
    <footer>
      <div class="u_button" @click="submit()">
        <div class="u_btn_lt">取消订单</div>
        <div class="u_btn_rt">提交订单</div>
      </div>
    </footer>
  </div>
</template>
...
<script>
  ...
  import orderAdressDetail from '../basic-views/order-address-detail.vue'
export default {
  components: {
    orderAdressDetail,
    orderItemDetail,
...
li{
  ...
  padding: .3rem .306667rem .506667rem .293333rem;
...
.u_button{
  ...
  .u_btn_lt{
    ...
    height: 1.066667rem;
    color: #375270;
    background:linear-gradient(90deg,#FFFFFF 0%,#DEEEFF 100%);
  ...
  .u_btn_rt{
    ...
    width:4.4rem;
    height:1.066667rem;
    background:linear-gradient(90deg,#5FB1FF 0%,#4F98FF 100%);
