<template>
  <div>
    ...
    <div class="result_tip" v-show="isError">{{msgStore}}{{msgQualification}}{{msgReceipt}}</div>
    <div class="title">审核不通过</div>
    <div class="title_tip">继续完善信息</div>
    <ul>
      <li>
        <group>
          <cell :class="[statusStore?'success':'failure']" title="店铺信息" :value="rltStore" :link="linkStore"></cell>
        </group>
        <group>
...
<script>
  import {validateApplyCheck, applyPermitSubmit} from '../../service/apply-restaurant.js'
export default {
  components: {
  ...
  data () {
    return {
      ...
      rltReceipt:'审核通过',
      linkStore:{path:'/restaurant-store-info',query:{reapply:true}},
      linkQualification:{path:'/restaurant-qualification-info',query:{reapply:true}},
      linkReceipt:{path:'/restaurant-receipt-info',query:{reapply:true}},
      msgStore:'',
      msgQualification:'',
      msgReceipt:'',
      isShowBtn:false,
  ...
  methods:{
    ...
    async getValidateApplyCheck(){
      ...
      this.syncStatus(res.data.store,res.data.aptitude,res.data.payinfo,res.data.storeResult,res.data.aptitudeResult,res.data.payResult);
      if(this.$route.query.isReapply){
        this.isShowBtn=true;
        if(sessionStorage.getItem('changeStore')){
    ...
    syncStatus(oriStatusStore,oriStatusQualification,oriStatusReceipt,MsgS,MsgQ,MsgR){
      switch(oriStatusStore){
        case 1:
          this.rltStore='待初审';
          this.statusStore=true;
          this.linkStore='';
        ...
        case 2:
          ...
          this.linkStore={path:'/restaurant-store-info',query:{reapply:true}};
          this.msgStore=MsgS;
          this.isError=true;
          break;
        case 4:
      ...
      switch(oriStatusQualification){
        ...
        case 2:
          ...
          this.linkQualification={path:'/restaurant-qualification-info',query:{reapply:true}};
      ...
      switch(oriStatusReceipt){
        ...
        case 2:
          ...
          this.linkReceipt={path:'/restaurant-receipt-info',query:{reapply:true}};
