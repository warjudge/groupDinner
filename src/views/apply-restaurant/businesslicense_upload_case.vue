<template>
  <div>
    ...
    <footer>
      <div class="u_button" @click="submit()">立即上传</div>
    </footer>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    submit(){
      if(this.isReapply){
        this.$router.push({path:'/restaurant-businessLicense-upload',query:{reapply:true}});
      }else if(this.isRefill){
        this.$router.push({path:'/restaurant-businessLicense-upload',query:{refill:true}});
      }else{
        this.$router.push({path:'/restaurant-businessLicense-upload'});
      }
    },
  },
}
